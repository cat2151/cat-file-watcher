...
## プロジェクト情報
名前: 
説明: # cat-file-watcher - Cat is watching your file -
...
`[auto_update]` セクション（省略可）で、git リポジトリの自動更新チェックを設定できます。設定するとバックグラウンドスレッドで定期的にリモートリポジトリの更新を確認します:

- `enabled` (省略可): `true` に設定すると、更新が見つかった場合に `git pull` を実行してプロセスを自動再起動します。デフォルトは `false`（ドライランモード: 更新があることを通知するだけで実際には pull・再起動しません）
- `interval` (省略可): 更新チェックの間隔。時間フォーマット（"1s", "2m", "3h" 等）で指定します。デフォルトは `"1h"`（1時間）

`[auto_update]` テーブル自体が設定ファイルに存在しない場合、自動アップデート用のバックグラウンドスレッドは起動されません。また、現在のブランチにリモートの追跡ブランチ（`@{u}`）が設定されていない場合は「更新なし」とみなし、`git pull` は実行されません。

```toml
[auto_update]
enabled = true   # false（デフォルト）にするとドライラン（通知のみ）
interval = "1h"  # 更新チェック間隔（デフォルト: 1時間）
```
