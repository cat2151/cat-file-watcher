...
- **Value**: An object with a `command` field containing the shell command to execute
  ...
  - `no_focus` (optional): If set to `true`, the command will be executed without stealing focus (default: `false`). **Windows only** - the command is launched asynchronously (the tool does not wait for it to complete) and the window is shown without activating it, preventing focus theft. Uses `shell=False` and splits the command by spaces. On non-Windows platforms, falls back to normal execution with a warning. Note: Shell features like pipes, redirects, and environment variable expansion are not available in this mode.
