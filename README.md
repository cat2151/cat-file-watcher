...
- `color_scheme` (optional): Terminal color palette. Supported values: `monokai` (default) or `classic`. To use custom colors, define a `[color_scheme]` table with `green`, `yellow`, and `red` specified as one of the following: `#RRGGBB`, `R,G,B`, `R;G;B`, `38;2;R;G;B`, or a full ANSI escape sequence such as `\x1b[38;2;255;60;80m`.
