...
実行されたコマンドの標準出力および標準エラー出力は、コンソールに表示されます:

- **成功時（終了コード 0）**: 標準出力がある場合、`Output: <出力内容>` として表示されます
- **失敗時（終了コード 0以外）**: `Error (exit code <コード>): <エラー内容>` として標準エラー出力が表示されます
- **エラーログファイル**: `error_log_file` を設定している場合、コマンド失敗時の詳細情報（エラーメッセージ、実行コマンド、標準エラー出力、スタックトレース）がログファイルに記録されます
